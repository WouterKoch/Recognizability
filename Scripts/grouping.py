...
def get_species_only(x):
    if len(str(x).split(" ")) != 2 or "×" in x:
        return None
    else:
        return x
...
def get_groups(path, threshold, numberOfSpecies, taxonlevel):
    columns = ['kingdom', 'phylum', 'class', 'order', 'family', 'genus', 'scientificName']

    df_verbatim = pd.read_csv(zipfile.ZipFile(path).open('verbatim.txt'), sep='\t', usecols=columns,
                              error_bad_lines=False)
    df_verbatim["scientificName"] = df_verbatim["scientificName"].apply(lambda x: get_species_only(x))
